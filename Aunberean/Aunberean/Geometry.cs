...
namespace Aunberean
{
        ...
        public static double CalculateHeadingWithLandblock(
            float startEW, float startNS,
            float targetEW, float targetNS)
        {
            uint origin = GetLandblockFromCoordinates(startEW, startNS);

            var startOffset = LandblockOffsetFromCoordinates(origin, startEW, startNS);
            var targetOffset = LandblockOffsetFromCoordinates(origin, targetEW, targetNS);

            var deltaY = targetOffset.Y - startOffset.Y;
            var deltaX = targetOffset.X - startOffset.X;
